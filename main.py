...
def filter_data(data, frame):
    ...
    return dates, open_prices, high_prices, low_prices, close_prices, frame


def render_chart(chart_type, dates, opens, highs, lows, closes):
    import os
    ...
    x_labels = dates[-10:]  # show only last 10 dates
    data_slice = slice(-10, None)

    if chart_type == 1:
        chart = Line(style=custom_style, x_label_rotation=45, show_minor_x_labels=False)        
        chart.x_labels = dates
        chart.add('Close', closes)
        filename = 'line_chart.svg'

    elif chart_type == 2:
        chart = Bar(style=custom_style, width=1200, height=600, x_label_rotation=45, show_minor_x_labels=False)  
        chart.x_labels = dates
